...
|-------|--------------|-------------------|--------------|
| **debug** | `app-debug.apk` | Автоматически при каждом push в main | Тест на телефоне, APK Editor Studio |
| **release** | `app-release.aab` без подписи | Только вручную | Подписать локально и в Google Play |
| **release-signed** | подписанные APK + AAB | Только вручную (нужны Secrets) | Сразу в Google Play |
| **🚀 Auto Release** | подписанные APK + AAB **+ страница GitHub Release с changelog** | Автоматически при push тега `v*` | Публичные релизы для скачивания |
...
## 🚀 Автопубликация релизов по git-тегу

Самый удобный способ выпускать новые версии Nova. Один раз настроил Secrets (см. раздел выше) — и потом каждый релиз делается **одной командой**.

### Как это работает

1. Ты делаешь `git tag v1.0.0` и `git push --tags`
2. GitHub Actions автоматически:
   - Собирает подписанные APK + AAB
   - Переименовывает их в `nova-v1.0.0.apk` и `nova-v1.0.0.aab`
   - Генерирует **changelog** из git-коммитов с прошлого тега
   - Создаёт публичную страницу **GitHub Release** с прикреплёнными файлами
3. Любой пользователь по ссылке `github.com/твой-репо/releases` видит:
   - Список всех версий
   - Описание что нового
   - Кнопки **Download** для APK и AAB

### Пример

```bash
# работаешь над фичами, коммитишь как обычно
git commit -m "Добавил тёмную тему"
git commit -m "Исправил баг с уведомлениями"

# когда готов к релизу — создаёшь тег
git tag v1.0.0
git push origin v1.0.0

# через 10 минут на странице releases появится:
#   Nova v1.0.0
#   - Добавил тёмную тему (a1b2c3d)
#   - Исправил баг с уведомлениями (e4f5g6h)
#   [📎 nova-v1.0.0.apk] [📎 nova-v1.0.0.aab]
```

### Соглашения по версиям

- `v1.0.0` — обычный релиз (stable)
- `v1.0.0-beta` или `v1.0.0-rc1` — автоматически помечается как **Pre-release** (тире в названии = бета)
- Семвер: `vMAJOR.MINOR.PATCH` (например `v2.3.1`)

### Где смотреть готовые релизы

`https://github.com/{твой-username}/{репозиторий}/releases`

Эту страницу можно дать друзьям/тестерам — они скачают APK напрямую, без GitHub-аккаунта.
